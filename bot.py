...
def welcome(self):
    figlet = Figlet(font='slant')  # Kamu bisa pilih font lain juga: 'standard', 'big', dll
    text = figlet.renderText('Auto Claim/Daily Billion Network')
    watermark = figlet.renderText('Join our channel at: https://t.me/airdropseeker_offiial')

    # Lebar terminal untuk centering, bisa disesuaikan
    width = 80

    # Print teks besar hijau di tengah
    print(Fore.GREEN + Style.BRIGHT + "\n".join(line.center(width) for line in text.splitlines()))
    print(Fore.GREEN + Style.BRIGHT + "\n".join(line.center(width) for line in watermark.splitlines()))
    ...
    async def process_accounts(self, session_id: str, use_proxy: bool):
                ...
                for idx, session_id in enumerate(session_ids, 1):
                    if session_id:
                        self.log(
                            ...
                            f"{Fore.BLUE + Style.BRIGHT} {len(session_ids)} {Style.RESET_ALL}"
                            f"{Fore.CYAN + Style.BRIGHT}]{separator}{Style.RESET_ALL}"
                        )
                        await self.process_accounts(session_id, use_proxy)
